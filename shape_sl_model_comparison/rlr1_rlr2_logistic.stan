...
parameters{
  ...
  real <lower = 0, upper =1> rlr2_delta[J];
  real <lower = 0> rlr2_learning_rate[J];
  real <lower = 0, upper =I> rlr2_split[J];
  ...
  real <lower = 0, upper = I> rlr2_split_mean;
...
transformed parameters{
  ...
  for(j in 1:J){
    ...
    + normal_lpdf(rlr2_split|rlr2_split_mean,rlr2_split_var)
  }
  ...
  for(n in 1:N){
    ...
    if(pp[n] == 1){
      ...
      rlr2[n] = rlr2_initial[jj[n]] * (rlr2_delta[jj[n]] * exp((rlr2_learning_rate[jj[n]])*(ii[n] - rlr2_split[jj[n]]))) * rt_mu[n];
